...
특히 여러 MSA 서버를 운영한다면 같은 Redis 서버를 보게될 수 있는데, **이때 ObjectMapper 설정은 동일하게 가져가는게 불필요한 이슈를 피할 수 있다는게 결론이다.** 
불가피한 경우가 아니라면 같은 Redis 서버에 접근하는 로직들은 통일하자.  
...
꼭 필요한 케이스가 있지 않다면 통일해서 사용하고, 예외 상황이 있어서 필요하다면 그때가서 고려하는게 효율적인 것 같다.
