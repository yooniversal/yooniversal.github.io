...
하지만 이 방법은 `delay()` 같은 중단점을 만나 다른 스레드로 전환되는 경우 MDC에 넣어둔 값을 유지하지 못한다.<br>
아래에서 설명하는 *MDCContext()* 를 사용한다면 해결할 수 있다.<br>
...
이렇게 기존 context(=CoroutineContext)에 복사할 수 있는 건 MDCContext의 `updateThreadContext()`를 호출하기 
때문이다.<br>
해당 로직은 [kotlinx-coroutines-slf4j](https://github.com/Kotlin/kotlinx.coroutines/blob/master/integration/kotlinx-coroutines-slf4j/src/MDCContext.kt#L90)에서 참조할 수 있다.<br>
...
- [Kotlin/kotlinx.coroutines github: MDCContext.kt 코드](https://github.com/Kotlin/kotlinx.coroutines/blob/master/integration/kotlinx-coroutines-slf4j/src/MDCContext.kt)